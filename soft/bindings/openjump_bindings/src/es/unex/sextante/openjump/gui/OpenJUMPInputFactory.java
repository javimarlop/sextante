 ...
 * 
 */
public class OpenJUMPInputFactory
         extends
   ...
   public void createDataObjects() {
      ...
      final List layerables = m_Context.getLayerManager().getLayerables(Layerable.class);
   ...
   public IDataObject openDataObjectFromFile(final String sFilename) {
      ...
      if (sFilename.endsWith("shp")) {
         ...
         final Layer layer = new Layer("", layerManager.generateLayerFillColor(), fc, layerManager);
   ...
   private ITable openTable(final String sFilename) {
      ...
      try {
         ...
         for (int j = 0; j < numfields; j++) {
            final AttributeType type = AttributeType.toAttributeType(mydbf.getFieldType(j));
   ...
   protected Envelope getGeoReferencing(final String fileName,
                                        ...
                                        final WorkbenchContext context) throws IOException {
      ...
      if (env == null) {
         ...
         if (fileName.toLowerCase().endsWith(".tif") || fileName.toLowerCase().endsWith(".tiff")) {
            ...
            final FileSeekableStream fileSeekableStream = new FileSeekableStream(fileName);
            final TIFFDirectory tiffDirectory = new TIFFDirectory(fileSeekableStream, 0);
               ...
               if (availTags[i].getTag() == GeoTiffConstants.ModelTiepointTag) {
                  ...
                  if (doubles.length != 6) {
                     ...
                     context.getWorkbench().getFrame().warnUser(
                              "unsupported value for ModelTiepointTag (" + GeoTiffConstants.ModelTiepointTag + ")");
                     break;
                  }
            ...
            if ((tiePoint != null) && (pixelScale != null)) {
               ...
               if (pixelOffset == null) {
                  ...
                  upperLeft = new Coordinate(tiePoint.x - (pixelOffset.x * pixelScale.x), tiePoint.y
                                                                                          - (pixelOffset.y * pixelScale.y));
               }

               lowerRight = new Coordinate(upperLeft.x + (imageDimensions.x * pixelScale.x), upperLeft.y
            ...
            final Coordinate upperLeft = new Coordinate(gf.getXllCorner(), gf.getYllCorner() + gf.getnRows() * gf.getCellSize());
            final Coordinate lowerRight = new Coordinate(gf.getXllCorner() + gf.getnCols() * gf.getCellSize(), gf.getYllCorner());
            ...
            final Coordinate upperLeft = new Coordinate(ga.getXllCorner(), ga.getYllCorner() + ga.getnRows() * ga.getCellSize());
            final Coordinate lowerRight = new Coordinate(ga.getXllCorner() + ga.getnCols() * ga.getCellSize(), ga.getYllCorner());
