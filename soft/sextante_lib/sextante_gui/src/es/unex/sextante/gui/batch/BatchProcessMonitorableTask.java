...
public class BatchProcessMonitorableTask
   ...
   public void run() {
      ...
      for (int i = 0; i < m_Parameters.size(); i++) {
         ...
         m_sLog.append(Sextante.getText("Process") + " " + Integer.toString(i) + "\n");
         m_sLog.append(LINE);

         m_ProgressMonitor.setDescriptionPrefix("[" + Integer.toString(i + 1) + "/" + Integer.toString(m_Parameters.size()) + "]");

         try {
            int iPercent = (int) ((double) (i * 100) / (double) m_Parameters.size());
            iPercent = Math.min(Math.max(0, iPercent), 100);
   ...
   private void getAdditionalResults(final GeoAlgorithm alg) {
      ...
      for (int i = 0; i < ooSet.getOutputObjectsCount(); i++) {
         ...
         final String sDescription = out.getDescription();
